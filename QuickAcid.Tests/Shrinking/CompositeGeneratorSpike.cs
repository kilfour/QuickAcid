...
public class CompositeGeneratorSpike
{
    ...
    {
        // var script =
        //     from inputOne in "inputOne".Composable(MGen.Constant(42))
        //     from inputTwo in "inputTwo".Composable(MGen.Constant(0))
        //     from composed in "composed".Composed(MGen.Constant(new Composed() { One = inputOne, Two = inputTwo }))
        //     from spec in "spec".Spec(() => composed.One + composed.Two != 42)
        //     select Acid.Test;
        // new QState(script).TestifyOnce();
        // var report = new QState(script).ObserveOnce();
