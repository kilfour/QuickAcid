...
public class FeedbackShrinkingTests
{
    ...
    public void Spike()
    {
        ...
        Assert.NotNull(report);
        Assert.Equal(4, report.OfType<ReportExecutionEntry>().Count());
    }
    ...
    private QAcidRunner<Acid> GetRun(Generator<int> intGenerator)
    {
        return
            ...
            from _ in "ops".Choose(
                ...
                from withdraw in "account.Withdraw:withdraw".ActIf(
                    () => flagWrapper.Flag, () => account.Withdraw(withdrawAmount))
                select Acid.Test
            )
