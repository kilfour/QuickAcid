...
public class LoftysCrane<T, TContext>
{
    private readonly Bob<T, TContext> bob;
    private readonly string label;
    private readonly Maybe<QKey<T>> key;

    public LoftysCrane(Bob<T, TContext> bob, string label, Maybe<QKey<T>> key = default)
    {
        this.bob = bob;
        this.label = label;
        this.key = key;
    }
    public Bob<Acid, TContext> Now(Action<T> effect)
